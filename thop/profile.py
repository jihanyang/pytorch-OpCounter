...
def profile_acts(
    ...
    def dfs_count(module: nn.Module, prefix="\t") -> (int, int):
        total_ops, total_params, total_acts = module.total_ops.item(), 0, module.total_acts.item()
        ret_dict = {}
        for n, m in module.named_children():
            # if not hasattr(m, "total_ops") and not hasattr(m, "total_params"):  # and len(list(m.children())) > 0:
            #     m_ops, m_params = dfs_count(m, prefix=prefix + "\t")
            ...
            if m in handler_collection and not isinstance(
                ...
                m_ops, m_params, m_acts = m.total_ops.item(), m.total_params.item(), m.total_acts.item()
            else:
                m_ops, m_params, m_acts, next_dict = dfs_count(m, prefix=prefix + "\t")
            ret_dict[n] = (m_ops, m_params, next_dict)
            total_ops += m_ops
            total_params += m_params
            total_acts += m_acts
        # print(prefix, module._get_name(), (total_ops, total_params))
        return total_ops, total_params, total_acts, ret_dict

    total_ops, total_params, total_acts, ret_dict = dfs_count(model)
